...
import { fetchAdminProducts, removeFeatured } from '../../slices/adminProductSlice';
...
const Featured = () => {
    ...
    const addfeature = () => {
        navigate('/dashboard/add-new-feature');
    }
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            const response = await dispatch(removeFeatured(selectedProductId)).unwrap();
            if (response.status) {
                toast(<div className='flex center g5'> < VerifiedIcon /> Product removed from fetured product</div>, { duration: 3000, position: 'top-center', style: { color: 'rgb(0, 189, 0)' }, className: 'success', ariaProps: { role: 'status', 'aria-live': 'polite' } });
                setSelectedProductId(null);
                dispatch(fetchAdminProducts({ page, size, sort, type }));
            } else {
        ...
        <Fragment>
            ...
            <article className="sortCat">
                ...
                <div className="flex g5">
                    <button onClick={addfeature}>Add Featured Products</button>
                    ...
                    <div className="popup">
                        <form className="popup-wrapper" style={{ gap: '10px' }} onSubmit={handleSubmit}>
                            ...
                            <div className="flex wh g10" style={{ marginTop: '15px', justifyContent: 'space-between' }}>
                                <button type='submit' className="applyBtn" disabled={isDeleted}>{isDeleted ? 'Remove...' : 'Yes'}</button>
